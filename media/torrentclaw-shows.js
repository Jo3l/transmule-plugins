/**
 * TorrentClaw Popular Shows — media provider plugin.
 *
 * Fetches popular TV shows from the TorrentClaw public API.
 * Clicking download opens a modal that fetches ALL torrent variants
 * for that specific show via the search API, with rich metadata tags.
 */
...
const CACHE_TTL = {
  ...
  detail: 6 * 60 * 60 * 1000, // 6 hours for show torrents
};
...
async function fetchItems(params) {
  ...
  const items = rawResults
    .filter((item) => (item.contentType || item.type || "") === "show")
    .map(toMediaItem);
...
// ── Detail: fetch ALL torrents for a specific show ─────────────
...
async function detail(searchQuery) {
  ...
  const match = rawResults.find(
    (item) => (item.contentType || item.type || "") === "show",
  ) ?? rawResults[0];
...
export default {
  meta: {
    id: "torrentclaw-shows",
    name: "TorrentClaw Popular",
    icon: "mdi-shield-check",
    mediaType: "shows",
    description:
      "Popular TV shows from TorrentClaw — 30+ sources, TrueSpec quality scores, rich metadata tags.",
    version: "1.0.0",
    repository:
